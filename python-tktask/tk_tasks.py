...
def num_judge(num):
    try:
        num = float(num)
        if num < 0:
            tkinter.messagebox.showinfo('error', '数值小于0，不符合计算要求')
        return num
    except:
        tkinter.messagebox.showinfo('error', '输入不为数值')
#修改e0的可编辑状态
def changestate():
...
def savetxt():
    try:
        ...
        try:
            ...
            f.write('\n')
            del result[:]
            tkinter.messagebox.showinfo('sucees', '保存成功')
        except:
            tkinter.messagebox.showinfo('error', '无计算数据')
        # print('success')
